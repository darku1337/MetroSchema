...
if (SERVER) then
	...
	function ENT:FireGun()
		...
		netstream.Start(player.GetAll(), "nut_EntFire", self)
	...
	function ENT:FireGun()      
		local pos, ang = self.models.barrel:GetPos(), self.models.barrel:GetAngles()
		self.models.lever.offset = Vector( 7, 0, 0 )
		self:ShellEject()
		...
		dlight.DieTime = CurTime() + 0.1
      	...
      	local dir = -ang:Up()			
		for i=0,5 do
			local Smoke = emit:Add("particle/smokesprites_000"..math.random(1,9), pos+dir*20 )
			Smoke:SetVelocity(120*i*1.5*dir*(scale*1.5))
		...
		local aang = dir:Angle()
		for i=2, 4 do
			local Gas = emit:Add( "effects/muzzleflash"..math.random(1,4), pos+dir*20 )
			Gas:SetVelocity ( dir*i*450*scale/1.05  )
			Gas:SetDieTime( math.Rand(0.04, 0.06)  )
			Gas:SetStartAlpha( 80 )
			Gas:SetEndAlpha( 0 )
			Gas:SetStartSize( (33 - i*1.4)*scale )
			Gas:SetEndSize( (44 - i*1.3)*scale/2 )
